...
def call_status():
    try:
        ...
        if is_dropped and retry_count < MAX_RETRIES:
            ...
            logger.warning(f"🔄  Attempt #{retry_count} of {MAX_RETRIES} — calling DID {EXOTEL_DID_NUMBER}")
...
def trigger_redial(caller_number, retry_count):
    try:
        ...
        # ── From = DID (connects first → triggers flow → Genesys ready) ───────
        # ── To   = Customer (connected after Genesys is ready) ────────────────
        payload = {
            'From'                : EXOTEL_DID_NUMBER,   # DID first
            'To'                  : caller_number,        # Customer second
            'CallerId'            : EXOTEL_CALLER_ID,
            'TimeLimit'           : 3600,
            'WaitUrl'             : 'http://com.exotel.customer-audio.s3.amazonaws.com/ring.mp3',
            'WaitMethod'          : 'GET',
            'StatusCallback'      : (
